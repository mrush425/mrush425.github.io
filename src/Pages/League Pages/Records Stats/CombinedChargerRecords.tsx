...
// Types
// =============================================================
interface ChargerAggregateRecord {
  userId: string;
  teamName: string;
  yearsPlayed: number;
  totalScore: number;
...
// =============================================================
const aggregateChargerWithExtremes = (data: LeagueData[]): ChargerAggregateRecord[] => {
  const userIds = new Set<string>();
  data.forEach(ld => ld.users.forEach(u => userIds.add(u.user_id)));

  const results: Record<string, ChargerAggregateRecord> = {};
  ...
  Array.from(userIds).forEach(userId => {
    ...
    data.forEach(league => {
      ...
      const stats = SidebetMethods.Charger(league, true, false);
...
// =============================================================
const yearlyForUser = (data: LeagueData[], userId: string): YearlyChargerRow[] => {
  const out: YearlyChargerRow[] = [];
  data.forEach(league => {
    const userInLeague = league.users.find(u => u.user_id === userId);
    if (!userInLeague) return;
...
interface YearlyProps {
  data: LeagueData[];
  selected: ChargerAggregateRecord;
}
      ...
      <table className="leagueStatsTable detail-table">
        ...
        <tbody>
          {rows.map(r => (
            <tr key={r.year}>
              <td>{r.year}</td>
              <td>{r.chargerScore.toFixed(0)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
// Main component
// =============================================================
const CombinedChargerRecords: React.FC<OtherComponentProps & { minYears?: number }> = ({ data, minYears = 0 }) => {
  const [sortConfig, setSortConfig] = useState<SortConfig>({ key: 'averageScore', direction: 'descending' });
  const [selected, setSelected] = useState<ChargerAggregateRecord | null>(null);
  ...
  const { rows } = useMemo(() => {
    const all = aggregateChargerWithExtremes(data);
  ...
  if (rows.length === 0) {
    return (
      <div className="regular-season-records">
        <div className="notImplementedMessage">No Charger data found for the current filter (min years: {minYears}).</div>
      </div>
    );
  }
...
export default CombinedChargerRecords;
