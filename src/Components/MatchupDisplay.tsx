...
const MatchupDisplay: React.FC<MatchupDisplayProps> = ({
    ...
    const renderTeamInfo = (
        ...
        if(user.avatar){
            ...
            avatarUrl = user.metadata.avatar
              ...
              : require('./Images/Default Team.png'); // Fallback to the local image if no avatar
        }

        const score = secondWeek ? getUserScore(user, week)+getUserScore(user, secondWeek) : getUserScore(user, week);
          ...
          <>
            <th className="col-10">
              {avatarUrl && (
                <img
                  src={avatarUrl}
                  alt={`${user.metadata.team_name || defaultTeamName} avatar`}
    ...
    const renderTotalPointsRow = (matchup1: Matchup | undefined, matchup2: Matchup | undefined) => (
      ...
      <div>
        ...
        <table className="matchup-display-table">
          ...
          <tbody>
            ...
            {/* Show first week total only if there is a second week */}
